...
class ABACAuthService:
    ...
    def add_credential(self, cred, issuer_cert):
        # Users can post credentials, usually delegating a given
        # permission to a second principal.
        
        # Assume cred is a binary string of the .der file generated by creddy.
        # DER is always binary, and it seems like the one generated by creddy
        # is not directly understood by openssl, so I'm not sure how you would
        # get a text encoding of the attribute file (though for HTTP you don't
        # really need to.)
        
        #issuer = ABAC.ID(issuer_cert)
        
        self.ctx.load_attribute_chunk(cred)

        return
        
        # Move to the abac store (should be done after validating though).
        credfile = os.path.join(self.ABAC_STORE_DIR, "some_sensible_naming", self.ATTR_FILE_SUFFIX)
        shutil.move(tmpfilename, credfile)
